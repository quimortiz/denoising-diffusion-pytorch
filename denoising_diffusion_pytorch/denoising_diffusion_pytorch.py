...
class MLPNet(nn.Module):
    def __init__(
        ...
        self.lf = nn.Linear(hidden_dim, dim)

        self.l_tx_1 = nn.Linear(time_dim + hidden_dim, hidden_dim)
        self.l_tx_2 = nn.Linear(time_dim + hidden_dim, hidden_dim)
        self.l_tx_3 = nn.Linear(time_dim + hidden_dim, hidden_dim)
    ...
    def forward(self, x, time, x_self_cond=None):
        ...
        x = F.gelu(self.l1(x))
        x = self.l_tx_1(torch.cat([x, t], dim=-1))
        x = F.gelu(self.l2(x))
        x = self.l_tx_2(torch.cat([x, t], dim=-1))

        x = F.gelu(self.l3(x))
        x = self.l_tx_3(torch.cat([x, t], dim=-1))
...
class GaussianDiffusionMLP(Module):
    def __init__(
        self,
        model,
        *,
